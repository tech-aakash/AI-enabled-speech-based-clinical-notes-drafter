...
def extract_diseases(transcript: str) -> List[str]:
    ...
    response = client.chat.completions.create(
        ...
        messages=[
            {
                "role": "system",
                "content": "You are a medical assistant for a doctor. Extract only the diseases or clinical complaints mentioned in the transcript. Reply only with a valid Python list like: ['fever', 'cold']."
            },
...
"""
    response = client.chat.completions.create(
        model="gpt-4o",
        messages=[
            {"role": "system", "content": "You are a clinical assistant generating structured notes for doctors."},
...
# --- Auth Check ---
username = st.session_state.get("user_profile", {}).get("first_name", "User")
...
with col2:
    ...
    if doctor_audio:
        ...
        with st.spinner("Searching SNOMED CT for treatments..."):
            treatment_terms = [doc_transcript]  # Optionally use NLP to extract keywords
